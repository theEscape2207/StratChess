...
| `Run-Lint.ps1 [-Check Format\|Tidy] [-All] [-Fix]` | clang-format (CI blocks on it) and clang-tidy (advisory). Resolves both tools via `vswhere`; neither is on `PATH`. `-Fix` iterates to a fixpoint — clang-format is not idempotent in one pass |
