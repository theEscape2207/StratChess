...
- **Late move pruning (compiled out of the shipping engine) returns entry alpha from a completed
  fail-low and stores nothing.** A depth-2 null-window frame that skipped a late quiet move and then
  failed low returns `original_alpha`, fail-hard, and writes no TT entry — neither UPPER nor EXACT —
  because the bound would rest on moves it never searched. It is a selective result, not a proof: it
