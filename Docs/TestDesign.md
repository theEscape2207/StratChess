...
- Same-key store: the incoming entry is scored against the one it would replace — a deeper or
  equal-depth store wins, a quiescence store or a shallower one is declined, and an accepted store
  with no move keeps the move already there
