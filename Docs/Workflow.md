...
`Scripts\Compare-SearchProfile.ps1 -Before <profile exe> -After <profile exe>` reads those lines: it
shows where two profile builds' trees differ, pooled, by endgame group and per position, and refuses
a default build on either side.
...
**Code placement moves nps, and the build pins only its 64-byte alignment.** Two node-identical
builds run at different speeds when unrelated code ahead of the hot path changes size, because the
hot functions land on different cache lines, pages and branch-predictor slots. Measured on #556: a
...
```powershell
pwsh -ExecutionPolicy Bypass -File <main-checkout>\Scripts\Remove-Worktree.ps1 -Path <registered-worktree-path> -SyncMaster
```
