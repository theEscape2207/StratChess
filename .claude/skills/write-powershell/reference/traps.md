...
session. Dot-source only a deliberate shared library; each has a `$SelfTestCoverers` entry in
`Validate-PrePR.ps1`, which is the list. Take one via `Join-Path $PSScriptRoot`.
