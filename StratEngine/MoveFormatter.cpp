...
std::string MoveFormatter::ToShort(const Move& move, ePiece movPiece)
{
	...
	case MoveType::PROMOTION_QUEEN_CAPTURE:
		...
		return std::format("{}{}{}{}{}", g_cPieceNames[PieceHelper::AsPawn(movPiece)], strFrom,
		                   (move.flags() & MoveFlags::CAPTURE_BIT) ? 'x' : '-', strTo, piece);
	case MoveType::KING_CASTLE:
		return "0-0";
	case MoveType::QUEEN_CASTLE:
		return "0-0-0";
...
std::string MoveFormatter::ToVerbose(const Move& move, const Board& board)
{
	...
	case MoveType::CAPTURE:
		result = std::string(PieceHelper::FullName(movPiece)) + " captures on " + to;
		break;

	case MoveType::EP_CAPTURE:
		result = std::string(PieceHelper::FullName(movPiece)) + " captures en passant on " + to;
		break;

	case MoveType::KING_CASTLE:
		result = std::string(PieceHelper::FullName(movPiece)) + " castles kingside";
		break;

	case MoveType::QUEEN_CASTLE:
		result = std::string(PieceHelper::FullName(movPiece)) + " castles queenside";
		break;
...
Move MoveFormatter::FromUCI(std::string_view uci, const Board& board)
{
	...
	if (PieceHelper::IsKing(piece)) {
		...
		if (fileDiff == 2)
			return Move(from, to, MoveType::KING_CASTLE);
		if (fileDiff == -2)
			return Move(from, to, MoveType::QUEEN_CASTLE);
	}
