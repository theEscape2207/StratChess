...
class MoveGenerator final {
  public:
	// Computes all the pseudo legal piece capturing moves on the current board - no check for Check
	// here!
  ...
  private:
	...
	static void GeneratePawnCaptures(const Board& board, const BITBOARD* const bbBitBoards,
	                                 const GameInfo& info, MoveList& moveList, eColor color);

	static void GeneratePawnNormalMoves(const BITBOARD* const bbBitBoards, eColor color,
	                                    MoveList& moveList);

	static void GenerateOfficerMoves(const Board& board, const BITBOARD* const bbBitBoards,
	                                 MoveList& moveList, ePieceType piece, eColor color,
	                                 bool onlyCaptures);

	static void AddOfficerMoves(const Board& board, MoveList& moveList, BITBOARD bbAttack,
	                            eSquare from);
	...
	static void AddPawnCaptures([[maybe_unused]] const Board& board, MoveList& moveList,
	                            const BITBOARD*, Move pawnMove, eColor color);
	static void AddCastleMoves(const Board& board, MoveList& moveList, eColor color,
	...
	static bool IsEnemyPieceOnTarget(const BITBOARD* bbBitBoards, eColor color,
	                                 const Move& move) noexcept
	{
		return Bits::isAnyBitSet(bbBitBoards[ePiece::ALL_BLACK_PIECES - static_cast<bool>(color)],
		                         g_bbMask[move.to()]);
	}

	static BITBOARD GetOfficerAttackBoard(const BITBOARD* bbBitBoards, eSquare from,
	                                      ePiece piece) noexcept;
	static BITBOARD GetRookBitboard(const BITBOARD* bbBitBoards, eSquare from,
	                                eColor color) noexcept;
	static BITBOARD GetBishopBitboard(const BITBOARD* bbBitBoards, eSquare from,
	                                  eColor color) noexcept;
