...
class CMemLeakDetect
{
	public:
		...
		class CMapMem
		{
			...
			static inline void GetNextAssoc(POSITION& pos, LPVOID& rAddr, AllocBlockInfo& aInfo) {
			...
			static void InitHashTable(int preAllocEntries, BOOL flag) 	{
	...
	static BOOL cleanupSymInfo();
		...
		void symStackTrace(STACKFRAMEENTRY* pStacktrace) const;
	static void symStackTrace2(STACKFRAMEENTRY* pStacktrace);
		BOOL symFunctionInfoFromAddresses(ULONG fnAddress, ULONG stackAddress, char* lpszSymbol) const;
		BOOL symSourceInfoFromAddress(UINT address, TCHAR* lpszSourceInfo) const;
		BOOL symModuleNameFromAddress(UINT address, TCHAR* lpszModule) const;
