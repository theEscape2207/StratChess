...
template <> struct fmt::formatter<eColor> {
	...
	auto format(eColor c, fmt::format_context& ctx) const
	{
		return fmt::format_to(ctx.out(), "{}",
		                      static_cast<const char*>(c == WHITE ? "White" : "Black"));
	}
};
...
template <> struct fmt::formatter<ePiece> {
	...
	{
		static constexpr const char* names[] = {
		    "White Pawn",   "Black Pawn", "White Knight", "Black Knight", "White Bishop",
		    "Black Bishop", "White Rook", "Black Rook",   "White Queen",  "Black Queen",
		    "White King",   "Black King", "All White",    "All Black",    "??",
