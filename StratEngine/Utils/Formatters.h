...
template <> struct fmt::formatter<eColor> {
	...
	{
		return fmt::format_to(ctx.out(), "{}", static_cast<const char*>(c == WHITE ? "White" : "Black"));
	}
};
...
template <> struct fmt::formatter<ePiece> {
	...
	{
		static constexpr const char* names[] = {
		    "White Pawn", "Black Pawn", "White Knight", "Black Knight", "White Bishop", "Black Bishop",
		    "White Rook", "Black Rook", "White Queen",  "Black Queen",  "White King",   "Black King",
		    "All White",  "All Black",  "??",           "No Piece" // index 14 is unused, 15 is NO_PIECE
		};
	...
	auto format(MoveType m, fmt::format_context& ctx) const
	{
		const char* name = [](MoveType t) -> const char* {
			...
			case MoveType::PROMOTION_KNIGHT_CAPTURE:
				return "Promotion+Capture (Knight)";
			case MoveType::PROMOTION_BISHOP_CAPTURE:
				return "Promotion+Capture (Bishop)";
			case MoveType::PROMOTION_ROOK_CAPTURE:
				return "Promotion+Capture (Rook)";
			case MoveType::PROMOTION_QUEEN_CAPTURE:
				return "Promotion+Capture (Queen)";
			default:
				return "??";
			}
		}(m);
