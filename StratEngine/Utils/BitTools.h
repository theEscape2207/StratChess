// The code is a header file that defines a namespace `Bits` containing utility functions for
// manipulating bitboards, which are represented as 64-bit unsigned integers (`std::uint64_t`).

// Original code copyrighted by Eduardo Velasquez and Sveinn Sigfredsson, with modifications for
// C++20 and chess engine optimizations.
...
[[nodiscard]] constexpr BITBOARD setBitsConditionally(BITBOARD value, BITBOARD mask,
...
[[nodiscard]] constexpr BITBOARD setClearBits(BITBOARD value, BITBOARD add,
                                              BITBOARD remove) noexcept
{
...
static_assert(Bits::setBitsConditionally(0b1010, 0b0001, true) == 0b1011,
              "setBitsConditionally failed");
static_assert(Bits::setBitsConditionally(0b1011, 0b0001, false) == 0b1010,
              "setBitsConditionally failed");
