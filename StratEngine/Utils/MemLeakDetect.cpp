...
static CMemLeakDetect*	g_pMemTrace			= nullptr;
static _CRT_ALLOC_HOOK	pfnOldCrtAllocHook	= nullptr;
						 ...
						 int	lineNumber)
{
	...
	{
		g_pMemTrace->addMemoryTrace(reinterpret_cast<void *>(requestNumber), size, (char*)filename, lineNumber);
	}
	else
	if (allocType == _HOOK_REALLOC)
	{
		if (_CrtIsValidHeapPointer(userData))
		{
			...
			if (pCrtHead->nBlockUse == _IGNORE_BLOCK)
			{
	   		...
	   		g_pMemTrace->redoMemoryTrace(reinterpret_cast<void *>(requestNumber), reinterpret_cast<void *>(prevRequestNumber), size, (char*)filename, lineNumber);
		}
	}
	else
	if (allocType == _HOOK_FREE)
	{
		if (_CrtIsValidHeapPointer(userData))
		{
			...
			if (pCrtHead->nBlockUse == _IGNORE_BLOCK)
			{
	   		...
	   		g_pMemTrace->removeMemoryTrace(reinterpret_cast<void *>(requestNumber), userData);
		}
	}
END:
	// unlock the function
...
void CMemLeakDetect::redoMemoryTrace(void* addr,  void* oldaddr, DWORD asize,  char *fname, DWORD lnum)
{
	...
	if (m_AllocatedMemoryList.Lookup(oldaddr,static_cast<AllocBlockInfo &>(ainfo)))
	{
...
void CMemLeakDetect::removeMemoryTrace(void* addr, void* /*realdataptr*/)
{
	AllocBlockInfo ainfo;
	//
	if (m_AllocatedMemoryList.Lookup(addr,static_cast<AllocBlockInfo &>(ainfo)))
	{
...
void CMemLeakDetect::dumpMemoryTrace()
{
	...
	while(pos != m_AllocatedMemoryList.end())
	{
		...
		AfxTrace(buf);
		//
		m_AllocatedMemoryList.GetNextAssoc(pos, static_cast<LPVOID &>(addr), static_cast<AllocBlockInfo&>(ainfo));
...
void CMemLeakDetect::Init()
{
	  ...
	  m_pSymbol				= static_cast<PIMAGEHLP_SYMBOL>(GlobalAlloc( GMEM_FIXED, m_dwsymBufSize));
...
void CMemLeakDetect::symStackTrace(STACKFRAMEENTRY* pStacktrace ) const
{
...
void CMemLeakDetect::symStackTrace2(STACKFRAMEENTRY* pStacktrace )
{
	...
	while (FramePtr)
	{
		InstructionPtr = reinterpret_cast<ADDR *>(FramePtr)[1];
		...
		FramePtr				= reinterpret_cast<ADDR *>(FramePtr)[0];
	}
}
BOOL CMemLeakDetect::symFunctionInfoFromAddresses( ULONG fnAddress, ULONG /*stackAddress*/, LPTSTR lpszSymbol ) const
{
	DWORD             dwDisp	= 0;
	...
	if ( SymGetSymFromAddr( m_hProcess, static_cast<ULONG>(fnAddress), &dwDisp, m_pSymbol ) )
	{
...
BOOL CMemLeakDetect::symSourceInfoFromAddress(UINT address, TCHAR* lpszSourceInfo) const
{
	BOOL           ret = FALSE;
...
BOOL CMemLeakDetect::symModuleNameFromAddress( UINT address, TCHAR* lpszModule ) const
{
	BOOL              ret = FALSE;
	...
	if ( SymGetModuleInfo( m_hProcess, static_cast<DWORD>(address), &moduleInfo ) )
	{
