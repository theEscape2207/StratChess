...
{
	switch (type) {
	case ePlayerTypes::HUMAN:				return std::make_unique<PlayerHuman>(board);		// no depth necessary
	case ePlayerTypes::ALPHABETA:			return std::make_unique<AIBasic>(board, max_depth);
	case ePlayerTypes::ABITERATING:			return std::make_unique<ABIterative>(board, max_depth);
	case ePlayerTypes::AIAGENT:				return std::make_unique<AIAgent>(board, max_depth);
	...
	case ePlayerTypes::AI_PERPLEX:			return std::make_unique<AIPerplex>(board, max_depth);
