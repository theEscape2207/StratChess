...
class AIPerplex final : public PlayerAiBase {
  public:
	...
	void SetThreads(unsigned n) noexcept override
	{
		threads_ = std::clamp(n, 1u, 32u);
	}
	void StartNewGame() override;

	// Note: NOT to be called directly - only through Factory method (needed to be public due to
  ...
  private:
	...
	struct SearchTuning {
		...
		int delta_pruning_margin = 200; // centipawns — added to stand_pat + capture_value before
		                                // comparing to alpha in quiescence
		...
		bool lmr_enabled =
		    true; // kill-switch: set false to measure LMR impact via SimplePerfStats.txt

		// Null-move pruning tuning
		bool null_move_enabled = true; // enabled by default (validated via tests + self-play; see
		                               // .claude/plans/null-move-pruning.md)
	...
	struct SearchState {
		...
		SearchState()
		    : best_move(Move::EmptyMove()), best_score(0), depth_completed(0),
		      nodes_at_completed_depth(0), last_iteration_move(Move::EmptyMove()),
	...
	int pvs(ThreadData& td, int depth, int alpha, int beta, int ply, bool is_pv_node,
	        TranspositionTable& tt);
	int adjustScoreForGameState(ThreadData& td, bool moveFound, int ply, int best_value);
	int quiescence(ThreadData& td, int alpha, int beta, int depth_q, int ply,
	               TranspositionTable& tt);
	...
	// --------------
	// Quality assessment
	RejectionReason assess_iteration_quality(const IterationMetrics& metrics,
	...
	bool should_try_null_move(const ThreadData& td, int depth, int beta, int ply, bool is_pv_node,
	                          bool in_check) const;
	...
	void log_completed_iteration(const AIPerplex::IterationMetrics& metrics,
	                             const PVTable& pv_table) const;
	void log_aspiration_retry(int depth, int retry, int score, int alpha, int beta,
	                          bool fail_low) const;
  ...
  public:
	...
	// When disabled, silences the logger via spdlog::level::off (log helpers need only a
	// null-check).
	static void SetVerboseLogging(bool enabled) noexcept;
	static bool IsVerboseLoggingEnabled() noexcept
	{
	...
	const SearchTuning& tuning() const
	{
		return tuning_;
	}
  ...
  public:
	...
	[[nodiscard]] SearchResult GetLastResult() const noexcept
	{
		return last_result_;
	}
};
