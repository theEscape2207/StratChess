...
SearchResult AIPerplex::iterative_deepening(ThreadData& td, int max_depth, TranspositionTable& tt)
{
	...
	for (int depth = 1; depth <= max_depth; ++depth) {
		...
		metrics.interrupted = TimeLimitReached(); // Check on time expiry
...
int AIPerplex::pvs(ThreadData& td, int depth, int alpha, int beta, int ply, bool is_pv_node, TranspositionTable& tt)
{
	...
	// After the first TimeLimitReached() fires and latches the flag, this collapses
	// the entire call stack in O(depth) steps instead of O(tree_size).
	if (IsAborted())
		return GameValues::Draw;

	// Node-based limit polling: check every 1024 nodes to amortise chrono::now() cost.
	// On first expiry either check latches should_stop_; all future IsAborted() calls
	// above then return true for free. Only thread 0 ever polls — helper threads
	// increment their own td.nodes_since_check_ but rely solely on the IsAborted()
	// fast-path above (no chrono::now() calls off the main thread). The node limit
	// shares this cadence, so it stops at the first multiple of 1024 at or past the
	// budget, and counts this thread's nodes only.
	if (td.thread_id == 0 && (++td.nodes_since_check_ & 1023) == 0) {
		if (TimeLimitReached() || NodeLimitReached(td.nodes_searched + td.qnodes_searched))
			return GameValues::Draw;
	}
		...
		if (td.board.DoMove(move)) {
			...
			if (first_child) {
				...
				if (applyLMR) {
					...
					if (value > alpha && !TimeLimitReached())
...
int AIPerplex::quiescence(ThreadData& td, int alpha, int beta, int qsearch_budget, int ply, TranspositionTable& tt)
{
	...
	// Mirrors pvs(): only thread 0 polls the limits; helpers rely on the
	// IsAborted() fast-path above.
	if (td.thread_id == 0 && (++td.nodes_since_check_ & 1023) == 0) {
		if (TimeLimitReached() || NodeLimitReached(td.nodes_searched + td.qnodes_searched))
			return GameValues::Draw;
	}
