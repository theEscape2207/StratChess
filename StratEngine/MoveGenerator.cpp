...
void MoveGenerator::GeneratePawnCaptures(const Board& board, const BITBOARD* const bbBitBoards,
                                         const GameInfo& info, MoveList& moveList, eColor color)
{
	BITBOARD bbAttackRight = 0;
	BITBOARD bbAttackLeft = 0;
	if (color == eColor::WHITE) {
		// Laver i foerste omgang maalfelt-bitboards med alle venstre- og hoejreskraa traek for
		// boenderne
		bbAttackRight = (Bits::clearBits(bbBitBoards[ePiece::WHITE_PAWN],
		                                 g_bbFileMask[eFileNames::RIGHT_FILE]) >>
		                 7);
		bbAttackLeft = (Bits::clearBits(bbBitBoards[ePiece::WHITE_PAWN],
		                                g_bbFileMask[eFileNames::LEFT_FILE]) >>
		                9);
	} else {
		// Laver i foerste omgang maalfelt-bitboards med alle venstre- og hoejreskraa traek for
		// boenderne
		bbAttackRight =
		    (Bits::clearBits(bbBitBoards[ePiece::BLACK_PAWN], g_bbFileMask[eFileNames::RIGHT_FILE])
		     << 9);
		bbAttackLeft =
		    (Bits::clearBits(bbBitBoards[ePiece::BLACK_PAWN], g_bbFileMask[eFileNames::LEFT_FILE])
		     << 7);
	}
	...
	if (info.epSquare != NO_SQUARE) {
		bbAttackLeft = Bits::applyMask(
		    bbAttackLeft, bbBitBoards[ePiece::ALL_BLACK_PIECES - static_cast<int>(color)] |
		                      g_bbMask[info.epSquare]);
		bbAttackRight = Bits::applyMask(
		    bbAttackRight, bbBitBoards[ePiece::ALL_BLACK_PIECES - static_cast<int>(color)] |
		                       g_bbMask[info.epSquare]);
	} else {
		bbAttackLeft = Bits::applyMask(
		    bbAttackLeft, bbBitBoards[ePiece::ALL_BLACK_PIECES - static_cast<int>(color)]);
	...
	while (bbAttackLeft) {
		...
		eSquare from = static_cast<eSquare>(
		    to + (color == eColor::BLACK ? -7 : 9)); // FIXME: Add defines, constants whatever
	...
	while (bbAttackRight) {
		...
		auto from = static_cast<eSquare>(
		    to + (color == eColor::BLACK ? -9 : 7)); // FIXME: Add defines, constants whatever
...
void MoveGenerator::GeneratePawnNormalMoves(const BITBOARD* const bbBitBoards, eColor color,
                                            MoveList& moveList)
{
	...
	if (color == eColor::WHITE) {
		// Et skridt frem
		bbMoveOne =
		    Bits::clearBits((bbBitBoards[ePiece::WHITE_PAWN] >> ONE_ROW), bbBitBoards[ALL_PIECES]);
		...
		bbMoveTwo = Bits::clearBits(((bbBitBoards[ePiece::WHITE_PAWN] & MASK_RANK_2) >> TWO_ROWS),
		...
		bbMoveOne =
		    Bits::clearBits((bbBitBoards[ePiece::BLACK_PAWN] << ONE_ROW), bbBitBoards[ALL_PIECES]);
		// Sorterer sidste raekke fra
		bbMoveOne = Bits::clearBits(bbMoveOne, MASK_RANK_1);
		// To skridt frem - sorte boender kun fra startplacering, dvs spillets 7. raekke
		bbMoveTwo = Bits::clearBits(((bbBitBoards[ePiece::BLACK_PAWN] & MASK_RANK_7) << TWO_ROWS),
		                            bbBitBoards[ALL_PIECES]);
		// Sikrer os at midterfeltet ogsaa er frit
		bbMoveTwo = Bits::clearBits(bbMoveTwo, (bbBitBoards[ALL_PIECES] << ONE_ROW));
	}
...
void MoveGenerator::GenerateOfficerMoves(const Board& board, const BITBOARD* const bbBitBoards,
                                         MoveList& moveList, ePieceType piece, eColor color,
                                         bool onlyCaptures)
{
	...
	while (bbPiecesToMove) {
		...
		if (onlyCaptures) {
			// Reducerer bitboardet til felter, hvor modstanderens brikker staar
			bbAttack =
			    Bits::applyMask(bbAttack, bbBitBoards[ALL_FROM_COLOR + static_cast<int>(color)]);
		}
...
BITBOARD MoveGenerator::GetOfficerAttackBoard(const BITBOARD* bbBitBoards, eSquare from,
                                              ePiece piece) noexcept
{
	eColor color = PieceHelper::Color(piece);
	switch (piece) {
	case ePiece::WHITE_KNIGHT:
	case ePiece::BLACK_KNIGHT:
		return Bits::clearBits(g_bbKnightMoves[from],
		                       bbBitBoards[ALL_FROM_COLOR + static_cast<int>(color)]);
	...
	case ePiece::BLACK_KING:
		return Bits::clearBits(g_bbKingMoves[from],
		                       bbBitBoards[ALL_FROM_COLOR + static_cast<int>(color)]);
...
void MoveGenerator::AddPawnPromoteMoves(const BITBOARD* bbBitBoards, eColor color,
                                        MoveList& moveList)
{
	BITBOARD bbAttack = 0;
	...
	if (color == BLACK) {
		...
		bbAttack = Bits::clearBits(((bbBitBoards[ePiece::BLACK_PAWN] << ONE_ROW) & MASK_RANK_1),
		                           bbBitBoards[ALL_PIECES]);
	} else {
		bbAttack = Bits::clearBits(((bbBitBoards[ePiece::WHITE_PAWN] >> ONE_ROW) & MASK_RANK_8),
		                           bbBitBoards[ALL_PIECES]);
	}
...
// <param name="moveList">Collection of found moves</param>
// Computes all possible moves, filters so only all captures, en passants and promotes remains and
// returns unsorted
...
void MoveGenerator::AddOfficerMoves(const Board& board, MoveList& moveList, BITBOARD bbAttack,
                                    eSquare from)
{
	...
	while (bbAttack) {
		...
		// MoveType flag encodes whether it's a capture; captured piece is retrieved from the board
		// when needed.
		moveList.push(MoveFactory::MakeMove(from, to, moveType));
 ...
 *	1) The King or the Rook cannot have moved earlier in the game ** this could pose a problem by
 * Chaining UndoMove()'s
 ...
 */
// TODO: Det er vist overkill at teste om alle felterne Kongen flytter forbi er angrebet for hver
// eneste traekgenerering i stedet kunne man flytte det til DoMove()
void MoveGenerator::AddCastleMoves(const Board& board, MoveList& moveList, eColor color,
                                   const BITBOARD* bbBitBoards, const GameInfo& info)
{
	...
	static constexpr std::array<CastlingSide, 2> sides = {
	    {{ePiece::WHITE_KING, e1, eColor::BLACK, CastlingRights::WHITE_KINGSIDE, g1,
	      g_bbMask[f1] | g_bbMask[g1], g_bbMask[e1] | g_bbMask[f1] | g_bbMask[g1],
	      CastlingRights::WHITE_QUEENSIDE, c1, g_bbMask[d1] | g_bbMask[c1] | g_bbMask[b1],
	      g_bbMask[e1] | g_bbMask[d1] | g_bbMask[c1]},
	     {ePiece::BLACK_KING, e8, eColor::WHITE, CastlingRights::BLACK_KINGSIDE, g8,
	      g_bbMask[f8] | g_bbMask[g8], g_bbMask[e8] | g_bbMask[f8] | g_bbMask[g8],
	      CastlingRights::BLACK_QUEENSIDE, c8, g_bbMask[d8] | g_bbMask[c8] | g_bbMask[b8],
	...
	const eSquare sqFrom = Board::GetFirstPiece(bbBitBoards[static_cast<ePiece>(
	    KING + static_cast<int>(color))]); // There is only one king!!
	...
	if (info.castlingRights & side.kingsideFlag) {
		// Are these squares being attacked ? The king must not move away from, pass or move into a
		// check!
		if (!Bits::isAnyBitSet(attackBoard, side.kingsideAttackMask) &&
		    !board.IsOccupied(side.kingsideTransitMask)) {
			moveList.push(
			    MoveFactory::MakeMove(sqFrom, side.kingsideTarget, MoveType::KING_CASTLE));
		}
	}
	...
	if (info.castlingRights & side.queensideFlag) {
		if (!Bits::isAnyBitSet(attackBoard, side.queensideAttackMask) &&
			...
			moveList.push(
			    MoveFactory::MakeMove(sqFrom, side.queensideTarget, MoveType::QUEEN_CASTLE));
		}
	}
}
                                    ...
                                    const BITBOARD* bbBitBoards, Move move, eColor color)
{
	...
	if (IsEnemyPieceOnTarget(bbBitBoards, color, move)) {
		...
		} else { // Promotion Captures — add each of the 4 promotions (isCapture=true selects
			     // PROMOTION_*_CAPTURE type)
			moveList.push(
			    MoveFactory::MakePromotion(from, to, PieceHelper::AsPiece(QUEEN, color), true));
			moveList.push(
			    MoveFactory::MakePromotion(from, to, PieceHelper::AsPiece(ROOK, color), true));
			moveList.push(
			    MoveFactory::MakePromotion(from, to, PieceHelper::AsPiece(BISHOP, color), true));
			moveList.push(
			    MoveFactory::MakePromotion(from, to, PieceHelper::AsPiece(KNIGHT, color), true));
		}
	}
	// Otherwise it must be an en-passant capture
	else {
		[[maybe_unused]] const eSquare epWhere = SquareHelper::PreviousRow(to, color);
		assert(Bits::isAnyBitSet(bbBitBoards[BLACK - color],
		                         g_bbMask[epWhere])); // There must be an opponent pawn here
...
//			:   eColor color -
//***************************************
BITBOARD MoveGenerator::GetRookBitboard(const BITBOARD* bbBitBoards, eSquare from,
                                        eColor color) noexcept
{
...
//			:   eColor color -
//***************************************
BITBOARD MoveGenerator::GetBishopBitboard(const BITBOARD* bbBitBoards, eSquare from,
                                          eColor color) noexcept
{
...
// Description: Generates all possible CAPTURE type moves for the color and combines the attacked
// squares into the returned BTBOARD.
...
BITBOARD MoveGenerator::GetAttackBoard(const Board& board, eColor attackByColor) noexcept
{
	...
	const BITBOARD bbOwnPieces =
	    boards[ALL_FROM_COLOR +
	           static_cast<int>(attackByColor)]; // Current position of own pieces - used to mask
	                                             // out illegal moves
	...
	if (attackByColor == WHITE) {
		...
		bbAttackBoard =
		    (((boards[ePiece::WHITE_PAWN] & ~(g_bbFileMask[eFileNames::RIGHT_FILE])) >> 7) |
		...
		bbAttackBoard =
		    (((boards[ePiece::BLACK_PAWN] & ~(g_bbFileMask[eFileNames::RIGHT_FILE])) << 9) |
	...
	if (epSquare != NO_SQUARE) {
		...
		BITBOARD adjacentPawns =
		    GetAnyEnPassantAttackingPawns(boards.data(), attackByColor, epSquare);
		if (adjacentPawns) {
	...
	bbPiecesToMove = Bits::setBits(boards[ROOK + static_cast<int>(attackByColor)],
